...
function renderProducts(products) {
  products.map((val) => {
    $(`
    <div class="col-xl-5 j-between">
      <div class="item">
        <a><img src="${val.image}" alt=""></a>
        <p class="name-products">${val.name}</p>
        <p class="price-products">$${val.price}</p>
        <div class="input-amount flex  a-center">
          <span class="down-ic ic">-</span>
          <input class="amount" type="text"/>
          <span class="up-ic ic">+</span>
        </div>
        <button class="btn-add"><p>ADD TO CART</p></button>
      </div>
    </div>
    `).appendTo(".row");
  })
